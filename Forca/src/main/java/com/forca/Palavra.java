...
public class Palavra implements Comparable<Palavra> {
    ...
    public Palavra(String texto) throws Exception {
        ...
        if (texto == null || texto.isBlank())
            throw new Exception("Por favor, entre com um texto valido!");
    ...
    public int getPosicaoDaIezimaOcorrencia(int i, char letra) throws Exception {
        ...
        for (k = 0; k < this.getTamanho(); k++) {
            if (this.texto.charAt(k) == letra) {
                ...
                lis[j] = k;
                j++;
                System.out.printf("Achou na posicao: %3d \n", k);
            }

        }

        if (j == 0)
            throw new Exception("Letra não encontrada no texto");
        else {
            for (k = 0; k < j; k++) {
                if (k == i)
                    ret = lis[k];
    ...
    public boolean equals(Object obj) {
        ...
        if (this.texto != pala.texto)
            return false;

        if (this.getTamanho() != pala.getTamanho())
            return false;

        for (int i = 0; i < this.getTamanho(); i++) {
            if (this.texto.charAt(i) != pala.texto.charAt(i))
                return false;
        }
